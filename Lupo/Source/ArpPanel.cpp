...
ArpPanel::ArpPanel (AttachmentFactory* factory, Arpeggiator* arp )
{
    ...
    gateSlider.reset (new Slider ("gateSlider"));
    addAndMakeVisible (gateSlider.get());
    gateSlider->setRange (5, 99, 1);
    gateSlider->setSliderStyle (Slider::RotaryVerticalDrag);
    gateSlider->setTextBoxStyle (Slider::TextBoxBelow, false, 60, 20);
    gateSlider->addListener (this);
    gateSlider->setValue (80, juce::NotificationType::dontSendNotification);
    gateSlider->setBounds (32, 200, 80, 80);

    gateLabel.reset (new Label ("gateLabel", TRANS("Gate %")));
    addAndMakeVisible (gateLabel.get());
    gateLabel->setFont (Font (13.00f, Font::plain).withTypefaceStyle ("Regular"));
    gateLabel->setJustificationType (Justification::centred);
    gateLabel->setEditable (false, false, false);
    gateLabel->setColour (TextEditor::textColourId, Colours::black);
    gateLabel->setColour (TextEditor::backgroundColourId, Colour (0x00000000));
    gateLabel->setBounds (32, 284, 80, 20);
...
void ArpPanel::sliderValueChanged (Slider* sliderThatWasMoved)
{
    ...
    else if (sliderThatWasMoved == gateSlider.get())
    {
        float gatePercent = static_cast<float>(gateSlider->getValue());
        arp->setGateTime(gatePercent / 100.0f);
        if (factory) {
            auto* apvts = factory->getValueTreeState();
            if (apvts)
                if (auto* param = apvts->getParameter("arpGate"))
                    param->setValueNotifyingHost(apvts->getParameterRange("arpGate").convertTo0to1(gatePercent));
        }
    }
